...
export const getDashboardStats = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      const adminMainBalanceStats =
        await AdminMainBalanceModel.calculateStats();
  ...
  SUM(
    CASE 
      WHEN transaction_type = 'withdraw'
        ...
        AND ${transactions.affiliateId} IS NULL
      THEN amount
      ELSE 0
    END
  )
...
`,
    ...
    `,
      ...
      const [gameProvidersCount] = await db
        .select({
          totalGameProviders: sql<number>`COUNT(*)`,
      ...
      const [sportsProvidersCount] = await db
        .select({
          totalSportsProviders: sql<number>`COUNT(*)`,
      ...
      const totalPlayerCurrentBalance =
        await BalanceModel.getTotalPlayerCurrentBalance();

      const totalGGRAmount =
        Number(waggerList?.totalBetAmount || 0) -
        Number(waggerList?.totalWinAmount || 0);

      const totalPlayerDepositsWithBonus =
        Number(adminMainBalanceStats.totalPlayerDeposit || 0) +
        Number(transactionStats[0]?.totalBonusAmount || 0);

      const netCompanyHoldingsFromPlayers =
        Number(totalPlayerDepositsWithBonus || 0) -
