...
exports.BalanceModel = {
    async calculatePlayerBalance(userId, currencyId) {
        try {
            let whereConditions = [(0, drizzle_orm_1.eq)(transactions_1.transactions.userId, userId)];
            if (currencyId) {
                whereConditions.push((0, drizzle_orm_1.eq)(transactions_1.transactions.currencyId, currencyId));
            }
            const result = await connection_1.db
                .select({
                // BDT amounts (original logic)
                totalDeposits: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'deposit' 
              AND ${transactions_1.transactions.status} = 'approved' 
              THEN ${transactions_1.transactions.amount} + COALESCE(${transactions_1.transactions.bonusAmount}, 0) ELSE 0 END), 0)
          `,
                totalSpinBonus: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'spin_bonus' 
              AND ${transactions_1.transactions.status} = 'approved' 
              THEN ${transactions_1.transactions.amount} + COALESCE(${transactions_1.transactions.bonusAmount}, 0) ELSE 0 END), 0)
          `,
                totalWithdrawals: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'withdraw' 
              AND ${transactions_1.transactions.status} = 'approved' 
              THEN ${transactions_1.transactions.amount} ELSE 0 END), 0)
          `,
                totalWins: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'win' 
              AND ${transactions_1.transactions.status} = 'approved' 
              THEN ${transactions_1.transactions.amount} ELSE 0 END), 0)
          `,
                totalLosses: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'loss' 
              AND ${transactions_1.transactions.status} = 'approved' 
              THEN ${transactions_1.transactions.amount} ELSE 0 END), 0)
          `,
                pendingDeposits: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'deposit' 
              AND ${transactions_1.transactions.status} = 'pending' 
              THEN ${transactions_1.transactions.amount} + COALESCE(${transactions_1.transactions.bonusAmount}, 0) ELSE 0 END), 0)
          `,
                pendingWithdrawals: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'withdraw' 
              AND ${transactions_1.transactions.status} = 'pending' 
              THEN ${transactions_1.transactions.amount} ELSE 0 END), 0)
          `,
                // USD amounts (converted)
                totalDepositsUSD: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'deposit' 
              AND ${transactions_1.transactions.status} = 'approved' 
              THEN (${transactions_1.transactions.amount} + COALESCE(${transactions_1.transactions.bonusAmount}, 0)) / COALESCE(${transactions_1.transactions.conversionRate}, 1) ELSE 0 END), 0)
          `,
                totalSpinBonusUSD: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'spin_bonus' 
              AND ${transactions_1.transactions.status} = 'approved' 
              THEN (${transactions_1.transactions.amount} + COALESCE(${transactions_1.transactions.bonusAmount}, 0)) / COALESCE(${transactions_1.transactions.conversionRate}, 1) ELSE 0 END), 0)
          `,
                totalWithdrawalsUSD: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'withdraw' 
              AND ${transactions_1.transactions.status} = 'approved' 
              THEN ${transactions_1.transactions.amount} / COALESCE(${transactions_1.transactions.conversionRate}, 1) ELSE 0 END), 0)
          `,
                totalWinsUSD: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'win' 
              AND ${transactions_1.transactions.status} = 'approved' 
              THEN ${transactions_1.transactions.amount} / COALESCE(${transactions_1.transactions.conversionRate}, 1) ELSE 0 END), 0)
          `,
                totalLossesUSD: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'loss' 
              AND ${transactions_1.transactions.status} = 'approved' 
              THEN ${transactions_1.transactions.amount} / COALESCE(${transactions_1.transactions.conversionRate}, 1) ELSE 0 END), 0)
          `,
                pendingDepositsUSD: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'deposit' 
              AND ${transactions_1.transactions.status} = 'pending' 
              THEN (${transactions_1.transactions.amount} + COALESCE(${transactions_1.transactions.bonusAmount}, 0)) / COALESCE(${transactions_1.transactions.conversionRate}, 1) ELSE 0 END), 0)
          `,
                pendingWithdrawalsUSD: (0, drizzle_orm_1.sql) `
            COALESCE(SUM(CASE WHEN ${transactions_1.transactions.type} = 'withdraw' 
              AND ${transactions_1.transactions.status} = 'pending' 
              THEN ${transactions_1.transactions.amount} / COALESCE(${transactions_1.transactions.conversionRate}, 1) ELSE 0 END), 0)
          `,
                ...
                .leftJoin(schema_1.promotions, (0, drizzle_orm_1.eq)(transactions_1.transactions.promotionId, schema_1.promotions.id))
                .where((0, drizzle_orm_1.and)(...whereConditions))
                .groupBy(transactions_1.transactions.userId);
            if (result.length === 0) {
            ...
            const currentBalance = totalDeposits +
                totalSpinBonus +
                totalWins -
                totalWithdrawals -
                totalLosses;
            const currentBalanceUSD = totalDepositsUSD +
                totalSpinBonusUSD +
                totalWinsUSD -
                totalWithdrawalsUSD -
                totalLossesUSD;
    ...
    async calculateAllPlayerBalances(filters = {}) {
        try {
            let whereConditions = [];
            if (filters.userId) {
                whereConditions.push((0, drizzle_orm_1.eq)(transactions_1.transactions.userId, filters.userId));
            }
            if (filters.currencyId) {
                whereConditions.push((0, drizzle_orm_1.eq)(transactions_1.transactions.currencyId, filters.currencyId));
            }
            const whereClause = whereConditions.length > 0 ? (0, drizzle_orm_1.and)(...whereConditions) : undefined;
            const balanceQuery = (0, drizzle_orm_1.sql) `
        SELECT 
          t.user_id as userId,
      ...
      `;
            const result = await connection_1.db.execute(balanceQuery);
    ...
    async getTotalPlayerCurrentBalance() {
        try {
            const result = await connection_1.db
                .select({
                // BDT amounts (original logic)
        ...
        `,
                ...
                .from(transactions_1.transactions)
                .where((0, drizzle_orm_1.isNotNull)(transactions_1.transactions.userId));
            const row = result[0];
            // BDT amounts
            const totalSpinBonus = Number(row.totalSpinBonus);
            const totalDeposits = Number(row.totalDeposits);
            ...
            const totalCurrentBalance = totalDeposits +
                totalSpinBonus +
                totalWins -
                totalWithdrawals -
                totalLosses;
            const totalCurrentBalanceUSD = totalDepositsUSD +
                totalSpinBonusUSD +
                totalWinsUSD -
                totalWithdrawalsUSD -
                totalLossesUSD;
    ...
    async getCurrencyBalance(userId, currencyId) {
        try {
